...
const CardCampaign: React.FC<CardCampaignProps> = (props) => {
  ...
  return (
    <>
      <section className="">
        <article className="flex flex-wrap items-center justify-center gap-8 p-4">
          {props?.data?.dataCampaigns?.map((campaign: T_Campaign, index: number) => (
            <div key={index} className="max-w-sm shadow-lg rounded-lg overflow-hidden relative">
              ...
              <div className="p-3">
                ...
                <h1 className="mb-2 block h-9 overflow-hidden break-words text-sm font-semibold text-mineshaft line-clamp-2">
                  {campaign.title}
                </h1>
                <div className="mb-2">
